...
def delete_file_by_id(file_id):
    ...
    try:
        full_path = os.path.join(current_app.config["UPLOAD_FOLDER"], os.path.basename(file.file_path))
...
def replace_file_by_id(file_id, req):
    ...
    try:
        old_path = os.path.join(current_app.config["UPLOAD_FOLDER"], os.path.basename(file.file_path))
    ...
    file.file_path = f"/uploads/{new_filename}"
    db.session.commit()
...
def download_file_by_id(file_id):
    ...
    return send_from_directory(
        ...
        path=filename,
        as_attachment=True,
        download_name=file.file_name  # Flask >=2.0
    )
